...
(define (alpha-parameters alpha-exp)
  (cadr alpha-exp))
...
;;; Quick syntax for promises

(define (await? exp)
  (tagged-list? exp 'await))

(define (await-vars exp)
  (cadr exp))

(define (await-body exp)
  (cddr exp))

(define (await->wait exp)
  (define (await-reducer vars body)
    (if (null? vars)
        `(begin ,@body)
        `(wait (car vars)
           (lambda (,(car vars))
             ,@(await-reducer (cdr vars) body)))))
  (await-reducer (await-vars exp) (await-body exp)))
