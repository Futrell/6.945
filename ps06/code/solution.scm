...
;;; Problem 6.4
;;;
;;; The problem is that the breadth-first schedule doesn't appear to
;;; go through all the same options that depth-first does. After
;;; displaying all the options for a variable, it goes on to display
;;; the options for the next variable without varying the previous
;;; one. This happens because the later set!s are called within the
;;; continuation *after* the previous set!s, so the previous values
;;; are not reset. On the other hand in depth first search this is not
;;; a problem, since we fully explore the later variables before
